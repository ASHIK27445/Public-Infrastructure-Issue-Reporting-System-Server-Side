...
async function initializeAIServices() {
  try {
    console.log('🤖 Initializing ENHANCED AI Toxicity Detection System...');
    ...
    // console.log('   - Enhanced keyword pattern database');
    // console.log('   - Emergency pattern matching');
    // console.log('   - AI + Keyword hybrid detection');
    // console.log('   - Bangla/English bilingual support');
    
    // Initialize with LOWER threshold
    await toxicityClassifier.initialize(0.55, optimalLabels);
    
    console.log('✅ ENHANCED AI Toxicity System READY!');
  ...
  try {
    ...
    async function checkToxicity(text) {
      try {
        ...
        const result = await toxicityClassifier.classify(text);
        ...
        if (result.isToxic && result.details && result.details.length > 0) {
          console.log(`   Detected patterns:`);
          result.details.forEach((detail, idx) => {
            console.log(`     ${idx + 1}. ${detail.label} (${detail.score}) [${detail.source || 'unknown'}]`);
          });
        }
        
        // Special logging for previously missed cases
        const lowerText = text.toLowerCase();
        if ((lowerText.includes('what kind of') || 
            lowerText.includes('people like you') ||
            lowerText.includes('why is this still')) && 
            !result.isToxic) {
          console.log(`   ⚠️ WARNING: Previously missed pattern detected but classified as CLEAN`);
          console.log(`   ⚠️ Please check if this is correct. Score: ${result.score}`);
        }
    ...
    app.post('/comments/:issueId', verifyFBToken, async(req, res) => {
      try {
        ...
        const commentData = {
          ...
          commenterPhoto: user.photoURL || null,
          commenterRole: user.role,
          commentText: isToxic ? '[Content removed due to toxicity]' : commentText.trim(),
          originalText: isToxic ? commentText.trim() : null, // Store original for moderation
          isToxic: isToxic,
          toxicityScore: toxicityResult.score,
          toxicityDetails: {
            labels: toxicityResult.details || [],
            method: toxicityResult.method || 'tensorflow',
            model: toxicityResult.model || 'toxicity-v1'
          },
          moderated: isToxic,
          moderatedAt: isToxic ? new Date() : null,
          moderatedReason: isToxic ? 'toxic_content' : null,
        ...
        console.log(`📢 New comment added to issue ${issueId}`);
        console.log(`🧠 Toxicity check: ${isToxic ? 'TOXIC' : 'CLEAN'} (${toxicityResult.score})`);

        // Send notification for toxic comments
        if (isToxic && user.role === 'citizen') {
          // You can add admin notification logic here
          console.log(`🚨 Toxic comment from user: ${user.name} (${user.email})`);
          
          // Optional: Send to moderation queue
          // await moderationQueue.add({ commentId: commentData._id, userId: user._id });
        }
    ...
    app.post('/comments/:commentId/reply', verifyFBToken, async(req, res) => {
      try {
        ...
        const reply = {
          _id: new ObjectId(),
          repliedText: isToxic ? '[Content removed due to toxicity]' : replyText.trim(),
          originalText: isToxic ? replyText.trim() : null,
          replierId: user._id,
          replierName: user.name,
          replierPhoto: user.photoURL || null,
        ...
        console.log(`💬 New reply added to comment ${commentId}`);
        console.log(`🧠 Toxicity check: ${isToxic ? 'TOXIC' : 'CLEAN'} (${toxicityResult.score})`);
...
async function startServer() {
  // Initialize AI models (non-blocking)
